...
After v0.6.1 deploys, open `/setup` and save once to update the GitHub release
workflow. Then send v0.6.1 again to test the integrated Android build.


## v0.6.1 — Self-healing release/build pipeline

From v0.6.1 onward, before every normal ZIP release the Telegram Worker uses
...
The currently active v0.4.1 code cannot use features that only exist in v0.6.1
before v0.6.1 has been deployed.

1. Send `Azhand-v0.6.1.zip` once.
2. After the bot reports that the Worker updated, send `/build`.

`/build` reuses the same v0.6.1 ZIP already stored in KV, updates the GitHub
...
## v0.6.1 — D1-independent build recovery
...
1. Send `Azhand-v0.6.1.zip`.
2. Wait for Worker auto-deploy.
3. Send `/build`.

After v0.6.1 is active, normal future ZIP uploads auto-build on the first try.


## v0.6.1 — lossless workflow embedding
...
v0.6.1 stores the complete workflow as Base64 and decodes it at runtime.
...
The currently deployed pre-v0.6.1 Worker will keep writing the broken workflow
before every dispatch. Therefore deploy `worker.js` from v0.6.1 to Cloudflare
once. After that, send `Azhand-v0.6.1.zip` normally. Future workflow updates
are automatic again.


## v0.6.1 — Android JVM target fix
...
1. Send v0.6.1 so the Worker updates.
2. Add the four signing secrets from the signing bundle.
3. Send `/build` once.
4. Install the signed v0.6.1 from `/app-update/latest.apk`.
5. From v0.6.1 onward, the release app detects updates itself.


## v0.6.1 — automatic Android signing

No GitHub Android signing secrets are required.

On the first v0.6.1 build, GitHub Actions automatically creates the permanent
...
send v0.6.1, wait for Worker deploy, then send `/build` once. After that normal
future ZIP releases require no manual signing setup.


## v0.6.1 — release token pipeline hardening
...
## v0.6.1 — resident accounts + real D1 data
...
## v0.6.1 — updater reliability + automatic D1 migrations
...
## v0.6.1 — payments + management operations
...
## v0.6.1 — Manager panel JavaScript hotfix
